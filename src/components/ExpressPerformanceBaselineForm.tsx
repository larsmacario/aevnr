  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    {metrics.map((metric) => {
      ...
      return <div key={metric.key} style={{ padding: "13px", borderRadius: 14, background: M.card, border: `1px solid ${M.line2}` }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 10 }}><input aria-label={t("baseline.valueAria", { title: metric.title })} type="number" inputMode="numeric" min={0} max={metric.max} value={current?.value ?? ""} placeholder={metric.unit} onChange={(event) => { const number = event.target.value === "" ? undefined : Math.min(metric.max, Math.max(0, Math.round(Number(event.target.value)))); setMetric(metric.key, number === undefined || !Number.isFinite(number) ? undefined : { value: number }); }} style={{ width: 84, minHeight: 42, boxSizing: "border-box", borderRadius: 9, border: `1px solid ${M.line}`, background: M.bg, color: M.fg, padding: "0 10px", fontSize: 16, WebkitAppearance: "none" }} /><span style={{ color: M.mut, fontSize: TYPE.bodySm }}>{metric.unit}</span></div>
