...
type CoachProps = { recommendation: CoachRecommendation; generating?: boolean; reasons: string[]; onCheckin: () => void; onOpenTimer: () => void; onOpenRecovery: (section?: "protein" | "water" | "checkin") => void; onOpenExpress: () => void; onStartStrength?: () => void };

export function DashboardCoach({ recommendation, generating = false, reasons, onCheckin, onOpenTimer, onOpenRecovery, onOpenExpress, onStartStrength }: CoachProps) {
...
export function HealthspanDashboard({ domains }: { domains: HealthspanDomain[] }) {
  return <section style={{ marginTop: 18 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 8 }}>
      {domains.map((domain) => <div key={domain.id} style={{ padding: 14, background: M.card, border: `1px solid ${M.line2}`, borderRadius: 18, boxShadow: M.shadow }}><div style={{ color: M.fg, fontWeight: 650, fontSize: 14 }}>{domain.label}</div><div style={{ height: 4, borderRadius: 4, background: M.line2, margin: "12px 0 8px", overflow: "hidden" }}><div style={{ width: `${Math.round(domain.progress * 100)}%`, height: "100%", background: M.brand, borderRadius: 4 }} /></div><div style={{ color: M.mut, fontSize: 12, lineHeight: 1.35 }}>{domain.detail}</div></div>)}
    </div>
  </section>;
}
